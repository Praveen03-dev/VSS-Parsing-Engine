...
class VSSParser:
    ...
    def __init__(self, file_resolver: Dict[str, str] = None):
        """
    ...
    def _parse_node_data(self, node_key: str, node_details: Dict[str, Any], parent_path: str = "") -> List[SignalNode]:
        ...
        """
        # Removed: print(f"\n--- Parsing node: {node_key} (Parent: {parent_path}) ---")
        # Removed: print(f"  Raw node_details input to _parse_vss_attributes: {node_details}")
 
        # Extract VSS-specific attributes from the current node's details
        vss_attrs = self._parse_vss_attributes(node_details)
       
        # Removed: print(f"  Extracted vss_attrs before SignalNode creation: {vss_attrs}")
 
        # Determine the base type of the node (signal, branch, sensor, actuator, attribute)
        node_type = node_details.get("type", "branch")
 
        generated_nodes: List[SignalNode] = []
 
        # Check if this is a branch that needs instance expansion
        if node_type == "branch" and vss_attrs.get('vss_instances'):
            ...
            for instance_name_suffix, instance_path_segment in self._generate_instance_paths(node_key, instances_def):
                ...
                instance_node = SignalNode(
                    ...
                    **vss_attrs # Assign all parsed VSS attributes
                )
                ...
                for child_key_inner, child_value_inner in node_details.items():
                    if child_key_inner.startswith("#include"):
                        self._handle_include_directive(child_key_inner, child_value_inner, current_node=instance_node)
                    elif isinstance(child_value_inner, dict) and "type" in child_value_inner:
                        ...
                        for c_node in child_signal_nodes:
                            instance_node.children[c_node.name] = c_node # Attach to this instance's children
            ...
            for child_key_inner, child_value_inner in node_details.items():
                if child_key_inner.startswith("#include"):
                    self._handle_include_directive(child_key_inner, child_value_inner, current_node=main_node)
                elif isinstance(child_value_inner, dict) and "type" in child_value_inner:
